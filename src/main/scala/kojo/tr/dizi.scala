 ...
 *
 * 2.12 UYARLAMASI (önemli): masaüstü sürümünde `Diz` (collection.Seq) ve `Dizi`
 * (Seq) için AYRI implicit class'lar var. 2.12'de bu ikisi AYNI TÜR
 * (değişmez/değişebilir Seq ayrımı 2.13'te geldi), dolayısıyla ikisini de
 * tanımlamak "ambiguous implicit" hatası verir. Tek sınıf ikisini de kapsıyor.
 *
 * Aynı sebeple masaüstündeki `IterableMethods` ve `ImmutableIterableMethods`
 * buraya alınmadı: 2.12'de bir List/Vector için hem onlar hem Seq sınıfı
 * uygulanabilir olurdu. Seq sınıfı çocukların kullandığı her şeyi kapsıyor.
 *
 * `distinctBy` 2.13'te geldi; elle yazıldı.
 */
trait DiziYöntemleri extends TemelTürler {
  ...
  // 2.12'de Diz ile Dizi aynı tür; ad uyumluluğu için ikinci bir kapı
  object Diz {
    def apply[B](ögeler: B*): Diz[B] = ögeler.toSeq
    def unapplySeq[B](dizi: Diz[B]) = Seq.unapplySeq(dizi)
    def doldur[B](n1: Sayı)(f: Sayı => B) = Seq.tabulate(n1)(f)
  }
  ...
  implicit class DiziMetotları[T](d: Dizi[T]) {
    ...
    // 2.13'teki distinctBy'ın karşılığı
